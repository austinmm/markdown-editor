...
function update(e) {
    ...
    /*If a title is added to the document it will become the new title,
...
function selectionChanger(selection,operator,endoperator){
    ...
    var isApplied = selection.slice(0, 2) === operator
...
function save(code, name) {
    ...
    if (window.saveAs) {
        ...
        event.initMouseEvent("click", true, true,
                        window, 1, 0, 0, 0, 0, false,
                        false, false, false, 0, null);
        link.dispatchEvent(event);
    }
}

//Encodes the document title to avoid strange chars and spaces
...
function start() {
    ...
    if (window.location.hash) {
        ...
        if (h.slice(0, 5) === "view:") {
            setOutput(decodeURIComponent(
                escape(RawDeflate.inflate(atob(h.slice(5))))
                )
            );
